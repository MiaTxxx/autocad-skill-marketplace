...
中性示例：25×25 板、R5 圆角、四角 Ø3 孔与圆角同心、正六边形对边距 12（内切圆 Ø12）、中心 Ø12 与 Ø6 两圆。以下为**在本机 CAD 里实测回读**的值（不是手算值）：

| 对象 | 回读值 |
|---|---|
| 25×25、R5 圆角闭合轮廓 | `Length=91.4159`、`Area=603.5398`、`Closed=True` |
| 对边距 12 的正六边形 | `Length=41.5692`、`Area=124.7077` |
| 解析校验（独立算路） | 周长 `2(w−2r)+2(h−2r)+2πr = 91.4159`；面积 `w·h−(4−π)r² = 603.5398`；正六边形 `6·(af/√3) = 41.5692`、`(√3/2)·af² = 124.7077` |

回归方法：跑一次该规格的 `--dry-run`（两条算路互证）与 `--clear`（落图后回读 12/12），数值应与上表一致。
